...
def main():
    ...
    with col1:
        ...
        choice_religin_atti = ['very serious about it', 'somewhat serious about it', 'not too serious about it', 'laughing about it']
        religion_option = st.selectbox('What is your religion? ', choice_religin)
        if religion_option == '<select>':    
            religion_option = 'N/A'
        if religion_option not in ["Agnosticism", "Atheism", "Irreligion"]:
            # Attitude to religion
            atti_religion_option = st.select_slider('How seriously do you take your religion? (if any):', options = choice_religin_atti)
        else:
    ...
    with col2:
        ...
        speaks = st.selectbox("Preferred Languages:", choice_language)
        ethnicity = st.selectbox("Preferred Ethnicity:", choice_ethnicity)
        religion = st.selectbox("Preferred Religion:", choice_religin)
        sign = st.selectbox("Preferred Astrological Sign:", choice_sign)
    ...
    st.write('Your ethicity: ', ethnicity_option)
    ...
    input_dict = {'age': age_input,
        ...
        'job': job_option,
        'location': pd.NA,
        'offspring': have_kids_option + ', '+ want_kids_option,
        'pets': have_pets_option + ', ' + like_pets_option,
